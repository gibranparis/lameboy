...
/* slight upward nudge so it matches the gate’s visual center */
function useModalShift(){
  const calc = () => {
    const h = typeof window !== 'undefined' ? window.innerHeight : 800;
    return { micro: Math.round(Math.max(6, Math.min(16, h * 0.02))) };
  };
  const [s, setS] = useState(calc);
  useEffect(() => { const onR = () => setS(calc()); window.addEventListener('resize', onR); return () => window.removeEventListener('resize', onR); }, []);
  return s;
}

function CascadeOverlay({ durationMs = CASCADE_MS, centerUpPx = 0 }){
  ...
  useEffect(() => {
    ...
    const step = (t) => { if (start == null) start = t; const raw = Math.min(1, (t - start) / durationMs); setP(ease(raw)); if (raw < 1) rafId=requestAnimationFrame(step); else doneId=setTimeout(()=>setMounted(false),120); };
  ...
  return createPortal(
    <>
      ...
      <div aria-hidden style={{ position:'fixed', inset:0, zIndex:10001, pointerEvents:'none' }}>
        <div style={{ position:'absolute', left:'50%', top:'50%', transform:`translate(-50%, calc(-50% - 18vh + ${centerUpPx}px))` }}>
          <span style={{ color:'#fff', mixBlendMode:'difference', fontWeight:800, letterSpacing:'.08em', textTransform:'uppercase', fontSize:'clamp(11px,1.3vw,14px)' }}>
            LAMEBOY, USA
          </span>
        </div>
      </div>
...
export default function BannedLogin({ onProceed }){
  ...
  return (
    <div
      ...
      style={{
        gap:10,
        transform:`translateY(calc(${micro}px - 18vh))`, // pull up like the screenshot request
      }}
    >
      {cascade && <CascadeOverlay centerUpPx={micro} />}
      ...
      <pre
        ...
        style={{ margin:0, textAlign:'center', color:'var(--text)', font:'700 14px/1.6 ui-monospace, SFMono-Regular, Menlo, Monaco, Consolas, "Liberation Mono", monospace' }}
      >
        <span className="lb-seafoam code-comment">// </span>
        <span className="lb-white neon-black" style={{ fontWeight:900 }}>Lamebo<span>y</span><span className="lb-seafoam">.com</span></span>{'\n'}
        <span className="lb-seafoam code-comment">// </span><span className="banned-neon">is banned</span>{'\n'}
