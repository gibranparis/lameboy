  ...
  return (
    <span className="lb-word" onClick={onClickWordmark} style={{ cursor:'pointer' }} title="Launch butterfly">
      ...
      <style jsx>{`
        ...
        .lb-white{ color:#fff; font-weight:900; text-shadow:0 0 8px #fff,0 0 18px #fff,0 0 36px #fff,0 0 70px #fff; }
        .lb-seafoam{}
      `}</style>
    </span>
  );
...
function CascadeOverlay({ durationMs = CASCADE_MS }){
  ...
  useEffect(()=>{
    ...
    id=requestAnimationFrame(step); return ()=> id && cancelAnimationFrame(id);
  },[durationMs]);
...
export default function BannedLogin({ onProceed }){
  ...
  useEffect(()=>{ const id=setInterval(()=>setHiMsg(v=>v===HI?JP:HI),22000); return ()=>clearInterval(id); },[]);
  ...
  return (
    <div className="page-center" style={{ position:'relative', flexDirection:'column', gap:8 }}>
      ...
      {whiteout && !cascade && createPortal(<div aria-hidden style={{ position:'fixed', inset:0, background:'#fff', zIndex:10002, pointerEvents:'none' }}/>, document.body)}
        ...
        <div className="login-stack" style={{ display:'grid', justifyItems:'center', gap:10 }}>
          <div className="orb-row" style={{ marginBottom:-16, display:'grid', placeItems:'center' }}>
            <button
              ...
              onMouseDown={()=>{ clearPress(); pressTimer.current=setTimeout(()=>runCascade(()=>{}, {washAway:true}),650); }}
              onMouseUp={clearPress} onMouseLeave={clearPress}
              onTouchStart={()=>{ clearPress(); pressTimer.current=setTimeout(()=>runCascade(()=>{}, {washAway:true}),650); }}
          ...
          {!hideBubble && (
            <div style={{ minWidth:260, background:'rgba(20,20,26,.9)', border:'2px solid rgba(255,255,255,.10)', borderRadius:16, boxShadow:'0 6px 18px rgba(0,0,0,.28)', padding:'6px 8px' }}
                 role={view==='login'?'form':undefined} tabIndex={view==='login'?0:-1}>
              {view==='banned' ? (
                <pre className="code-tight" style={{ margin:0 }}>
                  ...
                  <span role="button" tabIndex={0} className="code-banned banned-trigger"
                  ...
                  <span className="nogap">
                    <span role="button" tabIndex={0} className="code-string code-link"
                ...
                <form onSubmit={(e)=>e.preventDefault()} style={{ display:'flex', flexDirection:'column', gap:4 }}>
                  <div className="code-row"><span className="lb-seafoam code-comment glow-seafoam">// login</span></div>
                  <div className="code-row">
                    ...
                    <input ref={emailRef} className="code-input code-input-violet" value={email} onChange={(e)=>setEmail(e.target.value)}
                           placeholder="you@example.com" inputMode="email" autoComplete="email" autoCapitalize="none" autoCorrect="off"
                           size={Math.max(1,(email||'').length)} style={{ minWidth:'18ch' }}/>
                    <span className="nogap"><span className="code-string neon-violet">"</span><span className="code-punc neon-violet">;</span></span>
                  </div>
                  <div className="code-row">
                    <span className="code-var neon-violet">phone</span>
                    <span className="code-op neon-violet">=</span>
                    <span className="code-string neon-violet">"</span>
                    <input className="code-input code-input-violet" value={phone} onChange={(e)=>setPhone(e.target.value)}
                           placeholder="+1 305 555 0123" inputMode="tel" autoComplete="tel"
                           size={Math.max(1,(phone||'').length)} style={{ minWidth:'16ch' }}/>
                    <span className="nogap"><span className="code-string neon-violet">"</span><span className="code-punc neon-violet">;</span></span>
                  </div>
          ...
          <button type="button" className={['ghost-btn','florida-link', floridaHot?'is-hot':''].join(' ')}
                  onClick={()=>{ setFloridaHot(true); setTimeout(()=>setFloridaHot(false),700); setHideBubble(false); setView(v=>v==='banned'?'login':'banned'); }}
                  onMouseEnter={()=>setFloridaHot(true)} onMouseLeave={()=>setFloridaHot(false)}
